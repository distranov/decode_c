...
typedef struct
{
// данные для формирования предложения 61705
...
} __attribute__ ((__packed__)) tms100_ram_struct;
...
// отказы dm1
 uint32_t dm1_spn;                     //
} __attribute__ ((__packed__)) bat100_ram_struct;
...
typedef struct   /**/
  ...
  } __attribute__ ((__packed__)) sma_ram_struct;

typedef struct          /**/
  {
    uint16_t volt;               // -uc 14 12  // mV     напряжение ячейки
    uint16_t bal_cur;                          // mA     ток балансировки
    int8_t   temp;                             // °C     температура
    uint8_t  bal_cell_task;                    // *10mAh задание на балансировку
  } __attribute__ ((__packed__)) cell_ram_struct;

typedef struct
{
 uint32_t           start;
 tms100_ram_struct  tms;
 bat100_ram_struct  bat;
 mb100_ram_struct   mb;
 sma_ram_struct     sma[14];
 cell_ram_struct cell[14][12];
 uint8_t            res[24];
 uint32_t           crc32;
